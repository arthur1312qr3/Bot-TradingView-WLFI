...
def set_leverage(symbol, leverage):
    ...
    if result and result.get('code') == '00000':
        log(f"✅ Alavancagem {leverage}x configurada")
        return True
    return False
...
def place_order(symbol, side, quantity):
    """CORRIGIDO: side agora é 'buy' ou 'sell'"""
    endpoint = '/api/v2/mix/order/place-order'
    
    # CORREÇÃO: Bitget usa 'buy'/'sell', não 'open_long'/'close_long'
    params = {
        'symbol': symbol,
        'productType': 'USDT-FUTURES',
        'marginMode': 'crossed',
        ...
        'tradeSide': 'open' if side == 'buy' else 'close',  # IMPORTANTE!
        'orderType': 'market'
    }
    
    log(f"📤 Ordem: {side} {quantity} {symbol}")
    result = bitget_request('POST', endpoint, params)
...
def is_duplicate_signal(action):
    """Previne sinais duplicados em 10 segundos"""
    with signal_lock:
...
def webhook():
    try:
        ...
        if is_duplicate_signal(signal_key):
            log("⏭️ Sinal duplicado ignorado (10s)")
        ...
        if action == 'buy' and market_position == 'long':
            ...
            log(f"📊 Comprar: {quantity} WLFI")
            
            # CORREÇÃO: usa 'buy' ao invés de 'open_long'
            success = place_order(symbol, 'buy', quantity)
            
            if success:
                return jsonify({'status': 'success', 'action': 'COMPRADO'}), 200
            return jsonify({'status': 'error'}), 500
        ...
        # VENDER
            ...
            if position > 0:
                # CORREÇÃO: usa 'sell' ao invés de 'close_long'
                success = place_order(symbol, 'sell', position)
                if success:
                    return jsonify({'status': 'success', 'action': 'VENDIDO'}), 200
                return jsonify({'status': 'error'}), 500
            else:
                log("⚠️ Sem posição")
...
if __name__ == '__main__':
    log("🚀 Bot WLFI Iniciado")
    log(f"⚙️ {TARGET_SYMBOL} | {LEVERAGE}x | 100% | MARKET")
